...
def _load_prompt_styles() -> dict[str, str]:
    try:
        ...
        print(f"[ialhabbal_VLLM] Prompt style load failed: {exc}")
...
class ialhabbal_VLLM_PromptEnhancer(ialhabbal_VLLMBase):
    ...
    def _load_text_model(self, model_name, quantization, device_choice):
        ...
        if not repo_id:
            raise ValueError(f"[ialhabbal_VLLM] Missing repo_id for text model: {model_name}")
        ...
        print(f"[ialhabbal_VLLM] Loading text model {model_name} ({quantization})")
...
NODE_CLASS_MAPPINGS = {
    "ialhabbal_VLLM_PromptEnhancer": ialhabbal_VLLM_PromptEnhancer,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "ialhabbal_VLLM_PromptEnhancer": "ialhabbal_VLLM Prompt Enhancer",
}

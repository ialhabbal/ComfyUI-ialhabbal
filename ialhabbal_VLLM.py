    ...
    if custom.exists():
        try:
            ...
            if isinstance(custom_vl, dict) and custom_vl:
                ...
                print(f"[ialhabbal_VLLM] Loaded {len(custom_vl)} custom VL models")
            if isinstance(custom_text, dict) and custom_text:
                HF_TEXT_MODELS.update(custom_text)
                print(f"[ialhabbal_VLLM] Loaded {len(custom_text)} custom text models")
            if isinstance(legacy, dict) and legacy:
                HF_VL_MODELS.update(legacy)
                print(f"[ialhabbal_VLLM] Loaded {len(legacy)} custom legacy models")
        except Exception as exc:
            print(f"[ialhabbal_VLLM] custom_models.json skipped: {exc}")
...
def normalize_device_choice(device: str) -> str:
    ...
    if device == "cuda":
        if not torch.cuda.is_available():
            print("[ialhabbal_VLLM] CUDA requested but not available, falling back to CPU")
            return "cpu"
        return "cuda"
        ...
        if ":" in device:
            try:
                ...
                if device_idx >= torch.cuda.device_count():
                    print(f"[ialhabbal_VLLM] CUDA device {device_idx} not available, using cuda:0")
                    return "cuda:0"
            except (ValueError, IndexError):
                print(f"[ialhabbal_VLLM] Invalid CUDA device format '{device}', using cuda:0")
                return "cuda:0"
        return device

    if device == "mps":
        if not (getattr(torch.backends, "mps", None) and torch.backends.mps.is_available()):
            print("[ialhabbal_VLLM] MPS requested but not available, falling back to CPU")
            return "cpu"
        return "mps"
...
def get_sage_attention_config():
    ...
    if arch_code >= 120:  # Blackwell
        ...
        print(f"[ialhabbal_VLLM] SageAttention: Using SM120 (Blackwell) FP8 kernel")
    elif arch_code >= 90:  # Hopper
        pv_accum_dtype = "fp32+fp32"
        attn_func = sageattn_qk_int8_pv_fp8_cuda_sm90
        print(f"[ialhabbal_VLLM] SageAttention: Using SM90 (Hopper) FP8 kernel")
    elif arch_code == 89:  # Ada Lovelace
        pv_accum_dtype = "fp32+fp32"
        attn_func = sageattn_qk_int8_pv_fp8_cuda
        print(f"[ialhabbal_VLLM] SageAttention: Using SM89 (Ada) FP8 kernel")
    elif arch_code >= 80:  # Ampere
        pv_accum_dtype = "fp32"
        attn_func = sageattn_qk_int8_pv_fp16_cuda
        print(f"[ialhabbal_VLLM] SageAttention: Using SM80+ (Ampere) FP16 kernel")
    else:
        print(f"[ialhabbal_VLLM] SageAttention not supported on SM{arch_code}")
        return None, None, None
...
def resolve_attention_mode(mode, force_sdpa=False):
    ...
    if mode == "sage":
        if sage_attn_available():
            return "sage"
        print("[ialhabbal_VLLM] SageAttention forced but unavailable, falling back to SDPA")
        return "sdpa"
    if mode == "flash_attention_2":
        if flash_attn_available():
            return "flash_attention_2"
        print("[ialhabbal_VLLM] Flash-Attn forced but unavailable, falling back to SDPA")
        return "sdpa"
    
    # Auto mode: try sage → flash → sdpa
    if sage_attn_available():
        print("[ialhabbal_VLLM] Auto mode: Using SageAttention")
        return "sage"
    if flash_attn_available():
        print("[ialhabbal_VLLM] Auto mode: Using Flash Attention 2")
        return "flash_attention_2"
    print("[ialhabbal_VLLM] Auto mode: Using SDPA")
    return "sdpa"
...
def set_sage_attention(model):
    ...
    if not attention_classes:
        print("[ialhabbal_VLLM] Could not import any attention classes for SageAttention patching")
        return
    ...
    if patched_count > 0:
        print(f"[ialhabbal_VLLM] SageAttention: Patched {patched_count} attention layers")
    else:
        print("[ialhabbal_VLLM] SageAttention: No compatible attention layers found to patch")
...
def enforce_memory(model_name, quantization, device_info):
    ...
    if needed * 1.2 > available:
        if quantization == Quantization.FP16:
            print("[ialhabbal_VLLM] Auto-switch to 8-bit due to VRAM pressure")
            return Quantization.Q8
        if quantization == Quantization.Q8:
            print("[ialhabbal_VLLM] Auto-switch to 4-bit due to VRAM pressure")
...
class ialhabbal_VLLMBase:
    def __init__(self):
        ...
        print(f"[ialhabbal_VLLM] Node on {self.device_info['device_type']}")
    ...
    def load_model(
        ...
        if force_sdpa and attention_mode in ["auto", "sage", "flash_attention_2"]:
            if is_prequantized_fp8:
                print("[ialhabbal_VLLM] FP8 model detected - forcing SDPA attention")
            elif is_bnb_quantization:
                print("[ialhabbal_VLLM] BitsAndBytes quantization detected - forcing SDPA attention")
        
        print(f"[ialhabbal_VLLM] Attention backend selected: {attn_impl}")
        ...
        if self.model is not None:
            print("[ialhabbal_VLLM] Clearing previous model from memory before loading new configuration...")
        self.clear()
        ...
        if is_prequantized_fp8:
            ...
            print(f"[ialhabbal_VLLM] Loading FP8 model to {target_device}...")
            ...
            if has_meta:
                print("[ialhabbal_VLLM] Model has meta tensors, materializing...")
                ...
                print(f"[ialhabbal_VLLM] Loading weights from {model_path}")
                try:
                    ...
                    if os.path.exists(index_file):
                        ...
                        print("[ialhabbal_VLLM] Detected sharded checkpoint, using HF load_sharded_checkpoint...")
                        load_sharded_checkpoint(self.model, model_path, strict=True)
                        print("[ialhabbal_VLLM] All shards loaded successfully")
                    else:
                        ...
                        print(f"[ialhabbal_VLLM] Loading weights from {state_dict_path}")
                        ...
                        try:
                            ...
                            print("[ialhabbal_VLLM] All weights loaded successfully")
                        except RuntimeError as e:
                            # If strict loading fails, try non-strict and warn
                            print(f"[ialhabbal_VLLM] Strict loading failed: {e}")
                            print("[ialhabbal_VLLM] Attempting non-strict loading...")
                            missing_keys, unexpected_keys = self.model.load_state_dict(state_dict, strict=False)
                            if missing_keys:
                                print(f"[ialhabbal_VLLM] Warning: Missing keys: {missing_keys}")
                            if unexpected_keys:
                                print(f"[ialhabbal_VLLM] Info: Unexpected keys (not loaded): {unexpected_keys}")
                except Exception as e:
                    print(f"[ialhabbal_VLLM] Error loading weights: {e}")
                    raise
            
            # Now move to target device
            print(f"[ialhabbal_VLLM] Moving FP8 model to {target_device}")
            self.model = self.model.to(target_device)
            self.model.eval()
            print(f"[ialhabbal_VLLM] FP8 model loaded on {target_device}")
        else:
            ...
            if attn_impl == "sage":
                print(f"[ialhabbal_VLLM] Loading {model_name} ({quant.value}, base=sdpa, will_patch=sage)")
            else:
                print(f"[ialhabbal_VLLM] Loading {model_name} ({quant.value}, attn={actual_attn_impl})")
        ...
        if attn_impl == "sage":
            try:
                ...
                print("[ialhabbal_VLLM] SageAttention enabled")
            except Exception as exc:
                print(f"[ialhabbal_VLLM] SageAttention patching failed: {exc}")
        ...
        if use_compile and device.startswith("cuda") and torch.cuda.is_available():
            try:
                ...
                print("[ialhabbal_VLLM] torch.compile enabled")
            except Exception as exc:
                print(f"[ialhabbal_VLLM] torch.compile skipped: {exc}")
...
class ialhabbal_VLLM(ialhabbal_VLLMBase):
    @classmethod
    def INPUT_TYPES(cls):
...
class ialhabbal_VLLM_Advanced(ialhabbal_VLLMBase):
    @classmethod
    def INPUT_TYPES(cls):
        models = list(HF_VL_MODELS.keys())
        default_model = models[0] if models else "Qwen3-VL-4B-Instruct"
...
NODE_CLASS_MAPPINGS = {
    "ialhabbal_VLLM": ialhabbal_VLLM,
    "ialhabbal_VLLM_Advanced": ialhabbal_VLLM_Advanced,
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "ialhabbal_VLLM": "ialhabbal_VLLM",
    "ialhabbal_VLLM_Advanced": "ialhabbal_VLLM (Advanced)",
}
